...
<Flex dir="vertical" wrap="wrap" style={{flexGrow:1}}>       
    {prospectsList?.map((prospect,i) =>(
        <Card style={{width:500,margin:5}}>
        {/* <Typography>{prospect.id}</Typography> */}
        <Typography> <b>Name :</b> {prospect.name}</Typography>
        <Typography><b>Phone number :</b> {prospect.phone}</Typography>
        {/* <Typography>{prospect.agentId}</Typography> */}
        {/* <Typography>{prospect.ventureId}</Typography> */}

        <Typography> <b>Address :</b> {prospect.address?.street}
    ...
    <Button onClick={onClickhandler} style={{background:"orange",color:"white",margin:10,borderRadius:7}}>+ New Prospect</Button>
</Flex>
    )
